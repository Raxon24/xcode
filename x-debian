...
def printc(rText, rColour=col.OKGREEN, rPadding=0):
    print ("%s ┌──────────────────────────────────────────┐ %s" % (rColour, col.ENDC)
    for i in range(rPadding) print ("%s │                                          │ %s" % (rColour, col.OKGREEN))
    print ("%s │ %s%s%s │ %s" % (rColour, " "*(20-(len(rText)/2)), rText, " "*(40-(20-(len(rText)/2))-len(rText)), col.OKGREEN)
    for i in range(rPadding) print ("%s │                                          │ %s" % (rColour, col.OKGREEN))
    print ("%s └──────────────────────────────────────────┘ %s" % (rColour, col.ENDC)
    print " "
...
printc("UPGRADE")
    ...
    printc("Preparing Installation", co.YELLOW)
    ...
    if rVersion in rVersions:
        printc("Adding repo: debian %s" % rVersion)
        os.system("sudo apt-key adv --recv-keys --keyserver hkp://keyring.debian.org:80 0x2404C9546E145360")
        os.system("sudo add-apt-repository -y 'deb [arch=amd64,arm64,ppc64el] http://mirror.lstn.net/mariadb/repo/11.1/debian %s main'" % rVersions[rVersion])
   ...
   printc("MARIADB")
    ...
    printc("Configuring MariaDB", col.YELLOW)
    rCreate = True
    ...
    if rRet != 0:
        while True:
            ...
            if rRet == 0: break
            else: printc("Invalid password! Please try again.", col.WARNING)
    os.system('sudo mariadb -u root%s -e "DROP DATABASE IF EXISTS xui; CREATE DATABASE IF NOT EXISTS xui;"' % rExtra)
    os.system('sudo mariadb -u root%s -e "DROP DATABASE IF EXISTS xui_migrate; CREATE DATABASE IF NOT EXISTS xui_migrate;"' % rExtra)
    os.system('sudo mariadb -u root%s xui < "/home/xui/bin/install/database.sql"' % rExtra)
    os.system('sudo mariadb -u root%s -e "CREATE USER \'%s\'@\'localhost\' IDENTIFIED BY \'%s\';"' % (rExtra, rUsername, rPassword))
    os.system('sudo mariadb -u root%s -e "GRANT ALL PRIVILEGES ON xui.* TO \'%s\'@\'localhost\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "GRANT ALL PRIVILEGES ON xui_migrate.* TO \'%s\'@\'localhost\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "GRANT ALL PRIVILEGES ON mysql.* TO \'%s\'@\'localhost\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "GRANT GRANT OPTION ON xui.* TO \'%s\'@\'localhost\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "CREATE USER \'%s\'@\'127.0.0.1\' IDENTIFIED BY \'%s\';"' % (rExtra, rUsername, rPassword))
    os.system('sudo mariadb -u root%s -e "GRANT ALL PRIVILEGES ON xui.* TO \'%s\'@\'127.0.0.1\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "GRANT ALL PRIVILEGES ON xui_migrate.* TO \'%s\'@\'127.0.0.1\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "GRANT ALL PRIVILEGES ON mysql.* TO \'%s\'@\'127.0.0.1\';"' % (rExtra, rUsername))
    os.system('sudo mariadb -u root%s -e "GRANT GRANT OPTION ON xui.* TO \'%s\'@\'127.0.0.1\';"' % (rExtra, rUsername))
   ...
   printc("CONFIGURE")
    ...
    printc("Configuring System", col.YELLOW)
    ...
    if not rHasAdmin:
        rCode = generate(8)
        os.system('sudo maariadb -u root%s -e "USE xui; INSERT INTO access_codes(code, type, enabled, groups) VALUES(\'%s\', 0, 1, \'[1]\');"' % (rExtra, rCode))
   ...
   printc("FINISHED")
    ...
    print(" ")
    printc("Your MariaDB credentials have been saved to:" col.YELLOW)
    printc(rPath + "/credentials.txt")
    print(" ")
    printc("Please move this file somewhere safe!", col.WARNING)
